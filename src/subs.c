...
static int _subs_selectcontext(cJSON *root,char *key,int type,cJSON *will){
...
static int _subs_process(struct mosquitto_db *db, struct _mosquitto_subhier *hier, const char *source_id, const char *topic, int qos, int retain, struct mosquitto_msg_store *stored, bool set_retain)
{
	...
	bool client_retain;
        ...
        if(stored->payload) {
         ...
         cJSON *message = cJSON_Parse((char *)stored->payload);
           /* 根据key 找value in message*/
         cJSON *realMessage = cJSON_GetObjectItem(message,"realMessage");    
	 cJSON *filter = cJSON_GetObjectItem(message,"filter");
         Gfilter = filter;
         if(realMessage){
	  stored->payload = realMessage->valuestring;
          int messageLen = strlen(realMessage->valuestring);
          stored->payloadlen = messageLen;
		}
        }
		...
		if(rc2 == MOSQ_ERR_ACL_DENIED){
			...
			if(leaf->context->is_bridge){
				...
				client_retain = false;
                      ...
                      if(Gfilter) {
                        if(leaf->context->will) {
                              
		          cJSON *will = cJSON_Parse(leaf->context->will->payload);         
                          cJSON *realWill = cJSON_GetObjectItem(will, "realWill");
                          if(realWill){
			    
			    leaf->context->will->payload = realWill->valuestring;
			    int  willLength = strlen(realWill->valuestring);
			    leaf->context->will->payloadlen = willLength;
                             }
                          cJSON *root = Gfilter->child;
                          ...
                          if(!root||!_subs_selectcontext(root,name,type,will)) {
